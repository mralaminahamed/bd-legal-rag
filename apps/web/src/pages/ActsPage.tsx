  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <StatCard
          icon="ti-books"
          label="Registered Acts"
          value={actsQ.isLoading ? <Skeleton className="h-7 w-8" /> : acts.length}
        />
        <StatCard
          icon="ti-check"
          label="Fully ingested"
          value={actsQ.isLoading ? <Skeleton className="h-7 w-10" /> : `${fullyIngested} / ${acts.length}`}
          tone="success"
        />
        <StatCard
          icon="ti-stack"
          label="Total chunks"
          value={actsQ.isLoading ? <Skeleton className="h-7 w-14" /> : totalChunks.toLocaleString()}
        />
      </div>
